...
;desenhar retas
		...
		mov		ax,0   ; x do ponto 1
		push		ax
		mov		ax,479  ; y do ponto 1
		push		ax
		mov		ax,639  ; x do ponto 2
		push		ax
		mov		ax,479  ; y do ponto 2
		push		ax
		call		line

		mov		byte[cor],branco_intenso
		mov		ax,639   ; x do ponto 1
		push		ax
		mov		ax,479  ; y do ponto 1
		push		ax
		mov		ax,639  ; x do ponto 2
		push		ax
		mov		ax,0  ; y do ponto 2
		push		ax
		call		line
	...
	mov		ax,639   ; x do ponto 1
	push		ax
	mov		ax,0  ; y do ponto 1
	push		ax
	mov		ax,0  ; x do ponto 2
	push		ax
	mov		ax,0  ; y do ponto 2
	push		ax
	call		line
...
movimento:
	...
	movimento_x:
    add di, [incremento_x]
	cmp di, 629					; Compara di com 629, se di>629 -> ZF = 0 e CF = 0
								;					  se di<629 -> zf = 0 e cf = 1
								;					  se di=629 -> zf = 1 e cf = 0
								; zf e cf são flags (zero flag e carry flag)
	jae	estorou_x				; Salta se o valor de cf for zero
								; Ou seja, salta se di>629
								; https://www.felixcloutier.com/x86/jcc
	cmp di, 11
	jc	estorou_x				; Salta se o valor de cf for 1
								; Ou seja, salta se di<0			 
    jmp movimento_y

	estorou_x:
		neg word [incremento_x]
		jmp movimento_y
	...
	mov		ax,di  ; posição do centro em x
	push		ax
	mov		ax,si  ; posição do centro em Y
	push		ax
    ...
    jne adelante
    jmp movimento ; se AL = 0 então nada foi digitado e a animação do jogo deve continuar
