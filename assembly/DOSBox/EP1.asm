...
;___________________________________________________________________________
;	FUNÇÃO DECIMAL_2_ASCII - INATIVA!
	;	decimal_2_ascii:
	;		; push's
	;					pushf
	;					push 		ax
...
;	FUNÇÃO PINTAR_PIXEL
		...
		ret 
;___________________________________________________________________________
;	FUNÇÃO PRINT_CORES_LEN - INATIVA!
		print_cores_len:
			; push's
				pushf
				push 		ax
...
;	FUNÇÃO SHOW_IMG - INATIVA!
		...
		;		push		di
		;		push		bp
		;	mov		cx,		0		; A imagem começa a ser printada em x0 = 0
		;	mov 	dx, 	80		; y0 = 80, referencia superior esquerda.
		;	mov		bx, 	0		; indice do termo na lista de cores file_cores
		;	jmp print_pixel
		;	
		;	pixel_position:
		;		inc		cx				; x++
		;		cmp		cx,		300		; Se x = 300 -> x = 0, e y++
		;		je	y_inc				; Se x = 300 -> pula para Y++ function
		;		jmp print_pixel
		;
		;	y_inc:
		;		inc		dx				; y++
		...
		;
		;	print_pixel:
		;		mov		ah, 	0ch 				; Configuração para printar um pixel
		;											; https://stanislavs.org/helppc/int_10-c.html
		;		mov		al, 	[bx+file_cores]		; Cor do pixel
		;		mov 	bh,		00h					; Page number
		;		int 	10h							; Pinta o pixel
		;		inc 	bx							; cor do prox pixel
		;		cmp 	bx,		90000				; 300x300
		;		jne 	pixel_position
		;	; pop's
		;		pop		bp
		;		pop		di
